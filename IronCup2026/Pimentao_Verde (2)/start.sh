...
DIR=`dirname $0`
# Use lib bundled in package if installed path does not exist (e.g. on competition machine)
LIBPATH=/home/andrelcol/local/cyruslib/lib
if [ x"$LIBPATH" = x ] || [ ! -d "$LIBPATH" ] || ! ls "${LIBPATH}"/librcsc.so* >/dev/null 2>&1; then
  LIBPATH="${DIR}/lib"
fi
if [ -d "$LIBPATH" ]; then
