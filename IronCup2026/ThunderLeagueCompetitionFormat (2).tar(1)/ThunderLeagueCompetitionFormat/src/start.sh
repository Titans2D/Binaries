...
player="${DIR}/thunder_player"
coach="${DIR}/thunder_coach"
teamname="ThunderLeague"
host="localhost"
port=6000
coach_port=""
debug_server_host=""
...
team_graphic="--use_team_graphic on --team_graphic_file ${DIR}/thunder_logo.xpm"
...
# If true, save terminal std::cout and std::cerr of each player into 11 individual .log files
generate_logs="false"

# Optional .logs prefix to its filename
logs_prefix=""
...
usage() {
  (
    ...
    echo "  -gl, --generate-logs         generate 11 .logs files"
    echo "  -lp, --logs-prefix PREFIX    appends a prefix to .logs filename"
...
if [ $number -gt 0 ]; then
  ...
  if [ X"${offline_mode}" != X'' ]; then
    ...
    if [ "${generate_logs}" = "true" ]; then
      $player ${opt} -g >"${logs_prefix}$(date +"%FT%H%M%S")_${teamname}_player1.log" 2>&1 &
      sleep 4 &
      $sleepprog $goaliesleep
    else
      $player ${opt} -g &
...
while [ $i -le ${number} ]; do
  ...
  if [ X"${offline_mode}" != X'' ]; then
    ...
    if [ "${generate_logs}" = "true" ]; then
      $player ${opt} >"${logs_prefix}$(date +"%FT%H%M%S")_${teamname}_player${i}.log" 2>&1 &
      sleep 4 &
      $sleepprog $sleeptime
    else
      $player ${opt} &
